...
func Init(commandPath string, rootTaskConfig *TaskDef, opts ...Opts) (*CobraApp, error) {
	...
	rootCmd.PersistentFlags().StringVarP(&(p.ConfigFile), "config_file", "c", "", "Path to config file")
	rootCmd.PersistentFlags().BoolVar(&(p.LogToStderr), "logtostderr", true, "write log messages to stderr")
	rootCmd.PersistentFlags().StringArrayVarP(&(p.ConfigContexts), "config_context", "x", []string{}, "Config context")
	rootCmd.PersistentFlags().StringArrayVarP(&(p.ConfigDirs), "config_dir", "d", []string{}, "Config dir")

	rootCmd.PersistentFlags().StringVarP(&(p.LogLevel), "log_level", "", "info", "Log level. One of: panic|fatal|error|warn|info|debug|trace")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorPanic), "log_color_panic", "", "red", "Log message color: panic")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorFatal), "log_color_fatal", "", "red", "Log message color: fatal")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorError), "log_color_error", "", "red", "Log message color: error")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorWarn), "log_color_warn", "", "red", "Log message color: warn")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorInfo), "log_color_info", "", "cyan", "Log message color: info")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorDebug), "log_color_debug", "", "dark_gray", "Log message color: debug")
	rootCmd.PersistentFlags().StringVarP(&(p.LogColorTrace), "log_color_trace", "", "dark_gray", "Log message color: trace")
